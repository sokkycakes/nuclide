...
Browser-playable wireframe of the Alien Swarm / L4D-style **BaseModUI** menu flow.

> **Checkpoint:** A frozen copy lives at [`../basemod-wireframes-foundation/`](../basemod-wireframes-foundation/) — use that to revert experiments.
>
> **L4D2 hybrid fork:** [`../basemod-wireframes-l4d2/`](../basemod-wireframes-l4d2/) — foundation UX + offline L4D2 `.res` manifests (validation + optional position overlay).

Layout control IDs match `Resource/UI/BaseModUI/*.res` names referenced in the Alien Swarm SDK source; behavior mirrors `CBaseModPanel::OpenWindow` and panel `OnCommand` handlers.

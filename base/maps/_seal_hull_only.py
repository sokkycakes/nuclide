#!/usr/bin/env python3
"""Option-B seal-only pass for a decompiled Quake1 .map.
...
3. APPEND a sealed hull box (6 slabs) to the existing worldspawn entity.

Unlike _optimize_crux.py this does NOT move any structural worldspawn brush
into func_detail and does NOT replace worldspawn with the hull. Every original
brush (worldspawn + func_detail) is preserved exactly; we only add 6 slabs to
worldspawn to close the leak so vis can compute a real PVS.
"""
import math
import re
...
def main():
    ...
    if world is None:
        raise SystemExit("no worldspawn entity found")
    ...
    # APPEND hull to existing worldspawn brushes (do NOT replace/move anything).
    world["brushes"].extend(seal)
    ...
    print(f"after : structural(worldspawn)={n_struct + 6} (added 6 hull slabs)  func_detail={n_detail}")

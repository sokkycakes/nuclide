...
- **HUD assets**: Figma/Expresso-exported PNGs at **`base/ui/`**. Shared pip elements use **`ui/pip.png`** (VFS). Godot prototype sound assets at **`base/sound/player/footsteps/*.wav`**, **`base/sound/player/airdodge.ogg`**, **`base/sound/player/hardlanding.ogg`**, **`base/sound/player/airstall.wav`**.

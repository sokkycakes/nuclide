...
- **Solved-problem docs**: Durable fix write-ups with YAML frontmatter live under **`docs/solutions/`** (organized by category, e.g. `runtime-errors/`). Check there before re-diagnosing a known issue (e.g. raw-map spawnpoint origin corruption, CSQC remote-origin).
...
- **WebCore ICU**: Playtest/runtime must ship **`resources/icudt67l.dat`** next to the exe (`ftewebcore.dll` calls `u_setDataDirectory(<exe>\resources)`). Omitting it AVs in `ftewebcore.dll` a few seconds after map load when the HUD parses `@font-face`. Source is `nuclide/Resources/icudt67l.dat` or the WebCore CMake copy at `bin/resources/`. `publish-staging.ps1` throws if WebCore is present and this file is missing. See **`docs/solutions/runtime-errors/webcore-hud-icu-data-missing.md`**.

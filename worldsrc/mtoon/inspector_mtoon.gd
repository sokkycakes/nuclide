...
class MToonProperty:
    ...
    func get_mtoon_tooltip_text() -> String:
...
class OutlineModeInspector:
    ...
    func _item_selected(option_idx: int) -> void:
        ...
        if option_idx == 0 and has_outline:
            set_mtoon_property("next_pass", get_edited_object_hack().next_pass.next_pass)
        if option_idx != 0 and not has_outline:
            ...
            set_mtoon_property("next_pass", next_pass)
        set_mtoon_property(get_edited_property(), option_idx)
...
class SpinInspector:
    ...
    func _value_changed(value: float) -> void:
        set_mtoon_property(get_edited_property(), x_input.value)
...
class ScaleOffsetInspector:
    ...
    func _value_changed(value: float) -> void:
        ...
        set_mtoon_property(get_edited_property(), new_val)

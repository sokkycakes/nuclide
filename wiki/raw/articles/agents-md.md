...
ingested: 2026-07-17
sha256: 0a091aeb79024b77966cea6c046b9371794af406c635c83d527fc2b99194c737
---
...
- Prefer **engine-side** looping title-menu music (not WebCore audio): playlist **`music/menu_boot`** then **`music/menu1`**, **`music/menu2`**, … looping back to **`menu_boot`**; advance even when classic Quake menus are on top.
...
- Prefer **WebCore** over **Slint** for the in-game HUD as well as menus (WebCore HUD port under **`base/data/web/hud/`**, mirroring **`engine/ui/hud.slint`**).
...
- **WebCore UI (in progress)**: FTE plugin **`webcore`** loads host **`ftewebcore.dll`** (build tree **`workspace/webcore-fte`**, MSVC build dir often **`C:\w\wc-fte`**). Default fullscreen menu is **`base/data/web/title-menu/`** (`webcore_menu_url` → **`fte://data/web/title-menu/index.html`**); H3 mockup under **`base/data/web/h3-main-menu/`**; in-game HUD under **`base/data/web/hud/`**; smoke pages under **`base/data/web/webcore-test/`**. Slint sources being ported live under **`engine/ui/`** (`title_menu.slint`, `hud.slint`, drafts). Menu host logic in **`m_webcore_menu.c`** (playlist via **`webcore_menu_music`** stem). Goal: fullscreen **`menu_t`**, auto plugin load, in-game HUD—not UltralightCore.
...
- **Godot prototype / additive worlds**: When the user mentions the **Godot prototype** or **protophase2**, reference code from **`D:\c drive\Godot\stiletto-protophase2`**. Longer-term map authoring interest: Godot→FTE additive (unsealed) worlds; handoff **`docs/godot-fte-additive-world-handoff.md`**.
